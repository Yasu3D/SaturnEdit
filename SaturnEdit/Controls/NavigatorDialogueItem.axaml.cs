    ...
    private async void ButtonPickAudioFile_OnClick(object? sender, RoutedEventArgs e)
    {
        try
        {
            ...
            else
            {
                ...
                string directoryPath = Path.GetDirectoryName(navigator.AbsoluteSourcePath) ?? "";
                ...
                UndoRedoSystem.CosmeticBranch.Push(new GenericEditOperation<string>(value => { NavigatorDialogue.AudioPath = value; }, NavigatorDialogue.AudioPath, localPath));
            }
        }
        catch (Exception ex)
        {
